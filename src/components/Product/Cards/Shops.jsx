...
export default function Shops({vendor,image}) {
  return (
    <div className=" mx-auto  w-full bg-contain bg-right bg-no-repeat rounded-md" style={{backgroundImage: `url(${image?image:img})`}}>
      <div className="flex flex-col gap-3 bg-[#aaaa9b8a] p-4 px-8 rounded-md">
        <h3 className="text-[#EA580C] text-[14px] font-medium">Only This Week</h3>
          {/* eslint-disable-next-line react/prop-types */}
        <h2 className="text-[#111827] text-[22px] font-bold text-justify">{vendor.storeName}&nbsp;&nbsp;&nbsp;
        </h2>
        ...
        <div>
            {/* eslint-disable-next-line react/prop-types */}
          <ContinueLink link={`/stores/${vendor.vendor}`} className={"bg-[#FFF]"} text={"Shop now"} />
        </div>
      </div>
    </div>
  );
