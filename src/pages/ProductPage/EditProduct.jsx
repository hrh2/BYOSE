        ...
        async function main() {
            try {
                ...
                if (result.message) {
                    ...
                    setImageArray(result.data.product.images);
                }
            } catch (error) {
    ...
    return (
        <form className={`flex flex-col gap-4 pb-6`} >
            <div className="flex flex-row gap-2 p-4 md:px-0 container mx-auto">
                ...
                <IoIosArrowForward />
                <a href={`/dashboard/product/edit/${productID}`}>Update Product</a>
            </div>
            <div className={`container mx-auto flex justify-between bg-white p-4`}>
                <h2 className={`text-black font-bold text-xl`}>Update Product <i className={` font-normal`}>{data.name}</i></h2>
                {!loader&&
                    <div className={`flex  gap-3`}>
                        <button type={`button`} onClick={(event) => handleOnSubmit(event, true)} className={`bg-[#166534] pt-2 px-2 text-white font-bold rounded-md hover:bg-green-600`}>Save Changes</button>
                    </div>
                }
            </div>
            {loader&&<div className={`flex align-middle justify-center items-center py-2 h-[50vh]`}>
                <PropagateLoader color={`#166534`}/>
            </div>}
            {!loader&&<div className={`container mx-auto grid md:grid-cols-9 grid-cols-1 gap-4 px-4 pt-3 bg-white`}>
